...
class RoleManager {
  ...
  async getCurrentUserId(): Promise<string | null> {
    try {
      // DataManagerからユーザーシードを取得
      const DataManager = await import('./data-manager').then(m => m.default);
      ...
      console.error('Current user ID retrieval error:', error);
      return null;
    }
  }
  ...
  async getCurrentUserRole(): Promise<UserRole | null> {
    try {
      ...
      // ロールを取得
      return await this.getUserRoleAsync(userId);
    } catch (error) {
      console.error('Current user role retrieval error:', error);
      return null;
    }
  }
