...
export default function ViewerTab() {
  ...
  // 人気の動画を取得 - ユーザー識別子を含める (YouTubeAPI形式で返される)
  const { data: popularVideos, isLoading: popularLoading } = useQuery<YoutubeVideoItem[]>({
  ...
  const handleWatchVideo = (videoId: string) => {
    console.log('動画再生リクエスト:', videoId);
    
    if (!videoId) {
      console.error('無効なvideoId:', videoId);
      return;
    }
    ...
    try {
      ...
      // 強制的にreactの状態更新を促す（古いReactパターン）
      setTimeout(() => {
        // 2回目の設定で確実に反映させる
        setActiveVideoId(prevId => {
          console.log('現在のID:', prevId, '新しいID:', videoId);
          return videoId;
        });
      }, 10);
      ...
      console.log('activeVideoId設定完了:', videoId);
    } catch (err) {
      console.error('ビデオ再生設定エラー:', err);
    }
  };
  ...
  return (
    <>
      ...
      {console.log('レンダリング時のactiveVideoId:', activeVideoId)}
      {activeVideoId ? (
        <VideoPlayer 
        ...
        <div className="mb-4 p-2 bg-gray-100 dark:bg-gray-800 rounded text-sm text-center">
          <p>動画を選択すると、ここに再生プレーヤーが表示されます</p>
        </div>
      )}
