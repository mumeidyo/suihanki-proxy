...
export default function AdminTab() {
  ...
  const banIpMutation = useMutation({
    mutationFn: async (banData: BanData) => {
      // 現在のユーザーロールを取得してリクエスト
      await roleManager.getCurrentUserRole();
      return apiRequest('POST', '/api/bans', {
  ...
  const unbanIpMutation = useMutation({
    mutationFn: async (ipAddress: string) => {
      // 現在のユーザーロールを取得してリクエスト
      await roleManager.getCurrentUserRole();
      return apiRequest('DELETE', `/api/bans/${ipAddress}`);
    },
  ...
  const deleteCommentMutation = useMutation({
    mutationFn: async (commentId: number) => {
      ...
      // ロールがなければエラー
      if (!userRole || (userRole !== 'admin' && userRole !== 'developer' && userRole !== 'leader')) {
        throw new Error('権限がありません。管理者、開発者またはリーダーのみがコメントを削除できます。');
      }
      
      // リクエストを実行
      return apiRequest('DELETE', `/api/board/comments/${commentId}`);
    },
    onSuccess: (_, commentId) => {
      // 掲示板データを再取得
      queryClient.invalidateQueries({ queryKey: ['/api/board/posts'] });
      
      // 特定の投稿のコメントデータも再取得するために、すべての関連クエリを無効化
      queryClient.invalidateQueries();
      
      toast({
        title: "コメントを削除しました",
        description: `${commentId}番目のコメントを削除しました。`,
      });
  ...
  const handleDeleteComment = () => {
    if (!commentId || isNaN(Number(commentId))) {
      toast({
        title: "エラー",
        description: "有効なコメント番号を入力してください。",
        variant: "destructive",
      });
      return;
    }
    ...
    // 削除前に確認
    if (confirm(`コメント番号 ${commentId} を削除してもよろしいですか？この操作は元に戻せません。`)) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="bg-orange-50 dark:bg-orange-900/30">
          <div className="flex items-center">
            <Trash className="h-6 w-6 mr-2 text-orange-500" />
            <CardTitle>コメント削除管理</CardTitle>
          </div>
          <CardDescription>
            掲示板の問題のあるコメントを番号で指定して削除します
          </CardDescription>
        </CardHeader>
        <CardContent className="pt-6">
          <div className="mb-6 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md flex items-start">
            ...
            <div>
              <h3 className="font-medium text-yellow-700 dark:text-yellow-400">注意</h3>
              <p className="text-sm text-yellow-600 dark:text-yellow-500">
                削除したコメントは元に戻せません。コメント番号を確認してから削除してください。
              </p>
            </div>
          </div>
          
          <div className="space-y-4">
            <h3 className="text-lg font-medium">コメント削除</h3>
            
            <div className="grid gap-4">
              <div>
                <Label htmlFor="comment-id" className="mb-1 block">コメント番号</Label>
                <div className="flex items-center space-x-2">
                  <Input
                    ...
                    type="number"
                    min="1"
                    value={commentId === '' ? '' : String(commentId)}
                    onChange={(e) => setCommentId(e.target.value ? parseInt(e.target.value) : '')}
                    className="flex-1"
                  />
                  <Button 
                    onClick={handleDeleteComment}
                    variant="destructive"
                    disabled={!commentId || isNaN(Number(commentId)) || deleteCommentMutation.isPending}
                  >
                    {deleteCommentMutation.isPending ? "削除中..." : "削除する"}
                  </Button>
                </div>
                <div className="text-xs space-y-1 mt-2">
                  <p className="text-gray-500">
                    ※コメント番号は各コメントの左側に表示されている「#〇〇」の番号です
                  </p>
                  <p className="text-amber-600 font-medium">
                    ※現在は一時的に「#1」から始まる表示上の番号ではなく、データベース上の実際のコメントIDを入力する必要があります
                  </p>
                </div>
              </div>
            </div>
