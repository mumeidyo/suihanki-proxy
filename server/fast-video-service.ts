...
import { getVideoInfoFromInvidious } from './invidious-api-service';
...
export async function getFastPlaybackUrl(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (cachedData) {
      ...
      console.log(`Using cached formats for playback of ${videoId}, audioOnly: ${isAudioOnly}`);
      
      // 最適なフォーマットを選択 (オーディオのみのリクエストを処理)
      let optimalFormat;
      
      if (isAudioOnly) {
        // 音声のみのフォーマットを優先
        const audioFormats = cachedData.formats.filter(
          (format: any) => format.hasAudio && (!format.hasVideo || (format as any).vcodec === 'none')
        ).sort((a: any, b: any) => (b.bitrate || 0) - (a.bitrate || 0));
        
        optimalFormat = audioFormats.length > 0 ? audioFormats[0] : 
                        localSelectOptimalFormat(cachedData.formats, isMobile);
      } else {
        // 通常のビデオフォーマット選択
        optimalFormat = localSelectOptimalFormat(cachedData.formats, isMobile);
      }
    ...
    // キャッシュにない場合は新たに取得
    console.log(`No valid cache for ${videoId}, fetching new formats (audioOnly: ${isAudioOnly})`);
    const formats = await getVideoFormatsWithFallback(videoId);
    
    // 最適なフォーマットを選択 (音声のみかどうかに応じて)
    let optimalFormat;
    
    if (isAudioOnly) {
      console.log('Selecting audio-only format');
      // 音声のみのフォーマットをフィルタリング
      const audioFormats = formats.formats.filter(
        (format: any) => format.hasAudio && (!format.hasVideo || format.vcodec === 'none')
      ).sort((a: any, b: any) => (b.bitrate || 0) - (a.bitrate || 0));
      
      // 音声のみのフォーマットがあればそれを使用、なければ通常のフォーマット選択
      optimalFormat = audioFormats.length > 0 ? audioFormats[0] : localSelectOptimalFormat(formats.formats, isMobile);
    } else {
      optimalFormat = localSelectOptimalFormat(formats.formats, isMobile);
    }
    ...
    // 音声のみのリクエストの場合はJSONレスポンス、そうでない場合はHTMLプレーヤー
    if (isAudioOnly) {
      console.log('Returning JSON response for audio-only request');
      res.json({
        videoId,
        title: formats.videoDetails.title,
        url: optimalFormat.url,
        quality: optimalFormat.qualityLabel || 'Audio',
        container: optimalFormat.container || 'mp4',
        hasAudio: optimalFormat.hasAudio,
        audioBitrate: optimalFormat.audioBitrate || optimalFormat.bitrate || 0
      });
    } else {
      // カスタムプレーヤーページを返す
      const html = getFastPlayerHtml(
