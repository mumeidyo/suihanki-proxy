/**
 * Spotifyプレーヤー用のHTMLテンプレート
 */
export function getSpotifyPlayerHtml(trackId: string, title: string, artist: string): string {
  return `<!DOCTYPE html>
<html lang="ja">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${title} - ${artist} | Spotify Player</title>
...
<body>
  ...
  <div class="player-container">
    ...
    <div class="player-message">
      <p>Spotifyの曲を直接再生するには、Spotify APIキーが必要です</p>
      <p>このデモプレーヤーは、実際のSpotify Webプレーヤーの見た目を模しています</p>
    </div>
  ...
  <script>
    // 実際のアプリケーションではSpotify Web Playback SDK等を使用して実装します
    document.getElementById('playButton').addEventListener('click', function() {
      alert('Spotifyの曲の再生には、Spotify APIの認証が必要です。このデモでは実装されていません。');
    });
  </script>
</body>
</html>`;
