...
import { initAudioCacheDirectory, startAudioCacheCleanupTask, handleAudioStream } from "./audio-cache-service";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // 音声キャッシュの自動クリーンアップタスクを開始
  startAudioCacheCleanupTask(5 * 60 * 1000); // 5分ごとにクリーンアップ
  ...
  // 新しい音声ストリーミングエンドポイント（キャッシュあり）
  // 動作: サーバーに音声を保存 → ストリーミング → 次の曲に移動したら削除
  app.get("/api/youtube/cached-audio/:videoId", async (req: Request, res: Response) => {
    try {
      const { videoId } = req.params;
      console.log(`キャッシュ型音声ストリーミング要求: ${videoId}`);
      
      // 音声キャッシュサービスにリクエストを渡す
      await handleAudioStream(req, res);
    } catch (error) {
      console.error(`音声キャッシュストリーミングエラー: ${req.params.videoId}`, error);
      // フォールバック: 通常のストリーミングAPIを使用
      res.redirect(`/api/youtube/stream-video/${req.params.videoId}?audio=true`);
    }
  });

  // YouTube視聴API - リダイレクト型（モバイル対応・キャッシュあり）
  app.get("/api/youtube/stream-video/:videoId", async (req: Request, res: Response) => {
    try {
      console.log(`Stream request for video ${req.params.videoId}`);
      const { videoId } = req.params;
      ...
      // 音声のみのリクエストの場合
      if (isAudioOnly) {
        console.log('Audio-only request, using bestaudio format');
      ...
      try {
        console.log(`Getting stream URL for ${videoId} with options:`, options);
        
        // yt-dlpでURLを取得
        const result = await youtubeDlExec(videoUrl, options);
        
        if (result && typeof result === 'string' && result.startsWith('http')) {
          console.log(`Stream URL obtained for ${videoId}, redirecting...`);
        ...
        try {
          ...
          const fallbackOptions = {
            getUrl: true,
            format: isAudioOnly ? 'bestaudio/best' : 'best[ext=mp4]/best',
            noCheckCertificates: true,
            noWarnings: true
          };
          
          const result = await youtubeDlExec(videoUrl, fallbackOptions);
  ...
  app.get("/api/youtube/get-direct-urls/:videoId", async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        // For Music player, prioritize audio formats
        const directUrl = await youtubeDlExec(`https://www.youtube.com/watch?v=${videoId}`, {
          getUrl: true,
          format: 'bestaudio/best', // For music player, prioritize audio formats
          noCheckCertificates: true,
          noWarnings: true,
        ...
        try {
          const simplifiedUrl = await youtubeDlExec(`https://www.youtube.com/watch?v=${videoId}`, {
            getUrl: true,
            format: 'bestaudio/best', // For music, prioritize audio
            noCheckCertificates: true,
            noWarnings: true
          });
  ...
  app.get("/api/youtube/popular", async (req: Request, res: Response) => {
    try {
      ...
      // ランダム値に基づいて決定された確率（40%の確率でデータベース使用か）
      const useDatabase = accessRandom < 0.4; // 40%の確率
      
      // ランダム値と地域に基づいてデータベースを使用するかどうか決定
      if (useDatabase) {
        console.log(`データベースから最近の人気動画を取得 (${limit}件)`);
        
        // ランダム値でソート順を決定（0, 1, 2のいずれか）
        const sortOrder = Math.floor(accessRandom * 3);
        const videos = await storage.getPopularVideos(limit, sortOrder);
        
        if (videos && videos.length > 0) {
          console.log(`${videos.length}件の人気動画を取得しました（ソート順: ${sortOrder}）`);
          
          // YoutubeVideoItem形式に変換
          const formattedVideos = videos.map(video => ({
            ...
            snippet: {
              ...
              channelTitle: video.channelTitle || 'チャンネル不明',
      ...
      // データベースで結果が得られなかった場合や、ランダム確率でyt-dlpを使用
      try {
        // アクセスごとに最新のトレンド動画を取得
        console.log(`Getting trending videos using yt-dlp approach at timestamp: ${timestamp}`);
        
        // アクセスごとにランダムにリージョンを選択（より多様なコンテンツを提供）
        const regionOptions = ['JP', 'US', 'GB', 'FR', 'KR'];
        const randomRegionIndex = Math.floor(Math.random() * regionOptions.length);
        const selectedRegion = regionOptions[randomRegionIndex]; 
        
        console.log(`アクセスごとにランダムに選択されたリージョン: ${selectedRegion} (元のリクエスト: ${region})`);
        
        const trendingVideos = await getTrendingVideos(limit, selectedRegion);
        ...
        if (trendingVideos && trendingVideos.length > 0) {
          ...
          const defaultPopularVideos = [
            ...
            },
            {
              ...
              snippet: {
                title: 'クリエイティブな音楽',
                description: '様々なジャンルのクリエイティブな音楽をお楽しみください。',
                thumbnails: {
                  default: { url: 'https://i.ytimg.com/vi/fQ_m5VLhqNg/default.jpg' },
                  medium: { url: 'https://i.ytimg.com/vi/fQ_m5VLhqNg/mqdefault.jpg' },
                  high: { url: 'https://i.ytimg.com/vi/fQ_m5VLhqNg/hqdefault.jpg' }
...
<body>
  ...
  // デフォルトサムネイル用エンドポイント
  app.get("/api/proxy/default-thumbnail", (_req: Request, res: Response) => {
    // Base64エンコードされた最小限のプレースホルダー画像（グレーの四角形）
    const base64Image = '';
  ...
  // 掲示板投稿削除API（管理者用）
  app.delete("/api/board/posts/:postId", async (req: Request, res: Response) => {
    try {
      // ユーザー権限を確認（管理者、開発者、リーダーのみ削除可能）
      const userRole = req.headers['x-user-role'] as string;
      console.log('投稿削除リクエスト - ユーザー権限:', userRole);
      if (!userRole || (userRole !== 'admin' && userRole !== 'moderator' && userRole !== 'developer' && userRole !== 'leader')) {
        return res.status(403).json({
          success: false,
          error: "権限がありません。管理者、開発者またはリーダーのみが投稿を削除できます。"
        });
      }
      ...
      console.log(`投稿ID: ${postId} の削除を試行します...`);
      ...
      // 投稿を削除（関連コメントも削除される）
      const result = await storage.deleteBoardPost(postId);
  ...
  // コメント削除API（管理者用）
  app.delete("/api/board/comments/:commentId", async (req: Request, res: Response) => {
    try {
      // ユーザー権限を確認（管理者、開発者、リーダーのみ削除可能）
      const userRole = req.headers['x-user-role'] as string;
      console.log('コメント削除リクエスト - ユーザー権限:', userRole);
      if (!userRole || (userRole !== 'admin' && userRole !== 'moderator' && userRole !== 'developer' && userRole !== 'leader')) {
        return res.status(403).json({
          success: false,
          error: "権限がありません。管理者、開発者またはリーダーのみがコメントを削除できます。"
        });
      }
      ...
      // デバッグ: すべての投稿とコメントを確認
      console.log('コメント削除デバッグ: すべての投稿とコメントを確認します');
      const allPosts = await storage.getAllBoardPosts(20, 0);
      
      console.log(`データベース内の投稿数: ${allPosts.length}`);
        ...
        for (const post of allPosts) {
          try {
            ...
            if (comments && comments.length > 0) {
              console.log(`投稿ID ${post.id} のコメント数: ${comments.length}`);
              foundComments = [...foundComments, ...comments];
              
              // このコメントIDが存在するか確認
              const targetComment = comments.find(c => c.id === commentId);
              if (targetComment) {
                console.log(`削除対象のコメントが見つかりました:`, targetComment);
              }
            }
          } catch (e) {
            console.error(`投稿 ${post.id} のコメント取得エラー:`, e);
          }
        }
      }
      
      console.log(`データベース内の総コメント数: ${foundComments.length}`);
      console.log(`データベース内のコメントID一覧: ${foundComments.map(c => c.id).join(', ')}`);
      console.log(`削除しようとしているコメントID: ${commentId}`);
      
      // 投稿に対する最新のコメントを取得（表示IDが画面上の番号と異なる可能性があるため）
      if (req.query.postId) {
        try {
          const postId = parseInt(req.query.postId as string);
          if (!isNaN(postId)) {
            const postComments = await storage.getPostComments(postId);
            console.log(`投稿ID ${postId} の最新コメント:`, postComments);
            
            // 表示IDを元にコメントを取得（1から始まる表示上の順番）
            if (postComments && postComments.length > 0) {
              ...
              // 削除にはコメントの表示番号（画面上の番号）を使用
              // コメントは1から始まる番号で表示される
              const displayIdx = commentId - 1; // 表示は1から始まるが配列は0から始まる
              if (displayIdx >= 0 && displayIdx < sortedComments.length) {
                const actualComment = sortedComments[displayIdx];
                console.log(`表示番号 ${commentId}番 は実際のコメントID ${actualComment.id} に対応します`);
                
                // 実際のコメントIDで削除
                const result = await storage.deleteBoardComment(actualComment.id);
                ...
                if (result) {
                  ...
                  return res.json({
                    success: true,
                    message: `${commentId}番目のコメントを削除しました`
                  });
                }
              }
            }
          ...
          console.error('投稿コメント取得エラー:', e);
        }
      }
      
      // 通常の方法で削除を試みる
      const result = await storage.deleteBoardComment(commentId);
      
      if (!result) {
        return res.status(404).json({
          success: false,
          error: `指定されたコメント(${commentId}番)は存在しないか、すでに削除されています。利用可能なコメント番号: ${foundComments.length > 0 ? '1～' + foundComments.length : 'なし'}`
        });
      }
