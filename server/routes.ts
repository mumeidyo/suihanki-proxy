...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/youtube/search", async (req: Request, res: Response) => {
    try {
      ...
      // Method 3: Try using Invidious API for more reliable search (preferred)
      try {
        console.log("Trying Invidious API search approach...");
  ...
  app.get("/api/youtube/popular", async (req: Request, res: Response) => {
    try {
      ...
      // 方法1: Invidious APIを使用してトレンド動画を取得
      try {
        console.log('Getting trending videos from Invidious API');
        const trendingVideos = await getTrendingVideosFromInvidious(limit, region);
          ...
          for (const video of trendingVideos) {
            try {
              await storage.saveVideo({
                ...
                duration: video.contentDetails?.duration?.replace('PT', '').replace('H', ':').replace('M', ':').replace('S', '') || '0',
                viewCount: video.statistics?.viewCount || '0'
              });
            } catch (err) {
              console.error('Error saving trending video to storage:', err);
            }
          }
        ...
        console.error('Error getting trending videos from Invidious:', invidiousError);
      }
      
      // 方法2: ストレージから人気動画を取得（フォールバック）
