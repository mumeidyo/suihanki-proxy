...
export async function initializeStorage(): Promise<IStorage> {
  ...
  try {
    ...
    // Supabaseに接続できなかった場合は、PostgreSQLに接続を試みる
    if (databaseUrl) {
    ...
    // どちらも接続できなかった場合はメモリストレージを使用
    console.log('データベース接続が設定されていないか、接続に失敗しました。メモリストレージを使用します（再起動するとデータが失われます）。');
    _storage = new MemStorage();
    return _storage;
  } catch (error) {
    console.error('ストレージ初期化中にエラーが発生しました:', error);
    console.log('⚠️ エラーが発生したため、メモリストレージを使用します（再起動するとデータが失われます）。');
    _storage = new MemStorage();
    return _storage;
  }
}
